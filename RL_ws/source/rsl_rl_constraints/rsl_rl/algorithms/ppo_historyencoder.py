...
class PPO:
    ...
    def __init__(
        ...
        base_thresholds[4] = 0.15

        self.con_thresholds = base_thresholds / (1.0 - gamma)  # Pre-normalize
    ...
    def act(
        ...
        actor_obs = torch.cat([porprio_obs, v_est, priv_hat], dim=-1,)
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            # recon_loss = F.mse_loss(cenet_out["recon"], next_prop_obs_batch.detach())
            ...
            # Surrogate loss
            con_vals = cost_returns_batch.mean(dim=0)
            ...
            cost_gaes_tensor = (
                cost_advantages_batch - cost_advantages_batch.mean(dim=0, keepdim=True)
            ) / (cost_advantages_batch.std(dim=0, keepdim=True) + 1e-8)
