"""Export a B2 history-encoder checkpoint as a self-contained deployment bundle.

deploy.py는 CENet(cenet_encoder → mean_vel/mean_latent) 구조를 export한다.
이 파일(deploy2.py)은 ppo_historyencoder 구조 전용이다. 학습 시
    actor_obs = [prop_obs, velocity_estimator(history), privileged_encoder(history)]
로 배선되므로(on_policy_runner / ppo_historyencoder.act 참조), 배포용 forward도
CENet 대신 velocity_estimator + privileged_encoder를 proprio history에 적용한다.
입력은 여전히 prop_obs_history [B, frame_dim*history_length] 하나뿐이라
실기 인터페이스(pKIRO)는 동일하다.
"""
...
parser = argparse.ArgumentParser(description="Export the B2 history-encoder policy for pKIRO deployment.")
...
class B2DeploymentPolicy(torch.nn.Module):
    """Deterministic actor wrapper: history -> velocity/priv estimates -> action mean.

    학습 경로(ppo_historyencoder.act)와 동일하게 velocity_estimator와
    privileged_encoder를 proprio history 전체에 적용해 actor 입력을 구성한다.
    두 estimator 모두 입력이 prop_obs_history(= frame_dim*history_length)이며,
    출력은 각각 num_vel_latent / num_priv_encoder_latent 이다.
    """
    ...
    def __init__(
        ...
        self.velocity_estimator = copy.deepcopy(auxiliary_networks.velocity_estimator).cpu()
        self.privileged_encoder = copy.deepcopy(auxiliary_networks.privileged_encoder).cpu()
    ...
    def forward(self, prop_obs_history: torch.Tensor) -> torch.Tensor:
        ...
        velocity = self.velocity_estimator(prop_obs_history)
        priv_latent = self.privileged_encoder(prop_obs_history)
        actor_obs = torch.cat([prop_obs, velocity, priv_latent], dim=-1)
...
def _load_deployment_weights(runner: OnPolicyRunner, checkpoint: Path) -> None:
    ...
    # 배포에 실제로 쓰이는 head만 존재하면 충분하다. CENet 계열 파라미터는
    # ppo_historyencoder 체크포인트에 없을 수 있으므로 missing 목록에서 제외한다.
    required_prefixes = ("velocity_estimator.", "privileged_encoder.")
    missing_required = [
        key for key in auxiliary_result.missing_keys if key.startswith(required_prefixes)
    ]
    if missing_required:
        raise RuntimeError(
            "Checkpoint is missing history-encoder weights required for deployment: "
            f"{missing_required}"
        )
    if auxiliary_result.unexpected_keys:
        print(
            "[WARN] Ignoring unused auxiliary weights not needed for deployment: "
...
def _export_bundle(
    ...
    print(f"[INFO] Encoder: history_encoder (velocity_estimator + privileged_encoder)")
