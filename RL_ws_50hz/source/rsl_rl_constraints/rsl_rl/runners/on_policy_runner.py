...
class OnPolicyRunner:
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # CENet recon target은 clean obs — noisy history → clean 복원(denoising) 학습
                    self.alg.transition.next_prop_observations = extras["observations"]["prop_obs_clean"].to(self.device)
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        # 구버전 checkpoint에는 지금은 주석 처리된 estimator/encoder 가중치가 남아 있음
        # → unexpected key는 무시하되, missing key는 진짜 문제이므로 에러 유지
        aux_result = self.alg.aux_networks.load_state_dict(loaded_dict["aux_net_state_dict"], strict=False)
        if aux_result.unexpected_keys:
            print(f"[WARN] aux_networks: 구버전 checkpoint의 미사용 가중치 {len(aux_result.unexpected_keys)}개 무시함")
        if aux_result.missing_keys:
            raise RuntimeError(f"aux_networks에 없는 가중치: {aux_result.missing_keys}")
        # -----------------load normalizer----------------------------- #
        ...
        if load_optimizer:
            ...
            try:
                ...
                # 구버전 checkpoint는 미사용 네트워크 파라미터까지 optimizer에 있어 그룹 크기 불일치
                print("[WARN] aux optimizer state 불일치(구버전 checkpoint) — optimizer는 새로 초기화된 상태로 진행")
