...
class RBQEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.beta = (((-self._robot.data.projected_gravity_b[:, 0])) / 1.0).clamp(0.0, 1.0) * self.is_standing 
        ...
        idx = torch.tensor([0, 1, 4, 5, 8, 9], device=stand_joint_pos.device)
...
###################################
        ...
        self.flip_angle += (-self._robot.data.root_ang_vel_b[:, 1] * self.step_dt)
        self.flip_angle *= ~torch.any(is_contact[:, self._feet_contact_ids], dim=1)


        pitch_flip_rate = (-((self._robot.data.root_ang_vel_b[:, 1]).clamp(min=-10, max=0))) - (torch.sum(torch.square(self._robot.data.root_ang_vel_b[:, [0, 2]]), dim=1)) + (10 * self._robot.data.root_link_pose_w[:,2]) + (10*(self.flip_angle//2.0))
        ...
        feet_contact = torch.sum(is_contact[:, self._feet_contact_ids], dim=1)
        ...
        joint_vel_penalty = torch.sum(self.joint_limit_log_barrier(self._robot.data.joint_vel, torch.tensor(-25.0, device=self.device).expand(12), torch.tensor(25.0, device=self.device).expand(12), margin_ratio=0.3), dim=-1)
        ...
        torque_margin_ratio = torch.tensor([
            0.1, 0.1, 0.1, 0.1, #HR
            0.1, 0.1, 0.1, 0.1, #HP
            0.1, 0.1, 0.1, 0.1, #KP
        ], device=self.device)
        ...
        joint_torques_log_barrier = self.joint_limit_log_barrier(self._robot.data.computed_torque, -torque_limits, torque_limits, margin_ratio=torque_margin_ratio)
        ...
        stand_rewards = {
            ...
            "pitch_flip_rate": pitch_flip_rate * self.cfg.flip_reward_scale * 0.1 * self.step_dt,
        }
        ...
        biped_rewards = {
            "pitch_flip_rate": pitch_flip_rate * self.cfg.flip_reward_scale * self.step_dt,
            "no_feet_contact" : feet_contact * self.cfg.no_feet_contact_reward_scale * self.step_dt,
    ...
    def step(self, action: torch.Tensor): 
        ...
        self.extras["recoverability"] = (1-self.alpha.detach().clone()-(self.beta.detach().clone())).clamp(0.0, 1.0) * self.reset_terminated[:]
        self.extras["quad_weight"] = (self.alpha.detach().clone()-self.is_standing.detach().clone()).clamp(0.0, 1.0)
        self.extras["biped_weight"] = ((1-self.extras["recoverability"]+((self.beta.detach().clone()+self.alpha.detach().clone()))+torch.all(~is_contact[:, self._feet_contact_ids], dim=1).float())*self.is_standing.detach().clone()).clamp(0.0, 1.0)
