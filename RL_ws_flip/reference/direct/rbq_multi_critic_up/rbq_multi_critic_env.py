...
class RBQEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.beta = (((self._robot.data.projected_gravity_b[:, 0])-0.0) / 1.0).clamp(0.0, 1.0)
    ...
    def _apply_action(self):
        ...
        # kp_pitch_rate = 0.5  # tuning 필요

        # root_vel = self._robot.data.root_vel_w.clone()

        # yaw_only_quat = yaw_quat(self._robot.data.root_quat_w)

        # # current angular velocity in yaw-aligned frame

        # ang_vel_yaw_aligned = self._robot.data.root_ang_vel_w

        # current_pitch_rate = ang_vel_yaw_aligned[:, 1]
        # pitch_vel_err = target_pitch_rate - current_pitch_rate

        # # correction only in yaw-aligned pitch axis
        # delta_ang_vel_yaw_aligned = torch.zeros_like(ang_vel_yaw_aligned)
        # delta_ang_vel_yaw_aligned[:, 1] = kp_pitch_rate * pitch_vel_err

        # # convert correction back to world frame
        # delta_ang_vel_w = delta_ang_vel_yaw_aligned

        # # add correction on top of current world angular velocity
        # root_vel[:, 3:6] += delta_ang_vel_w

        # print("pitch rate:", current_pitch_rate[0].item())
        # print("pitch err :", pitch_vel_err[0].item())
        # print("quat      :", self._robot.data.root_quat_w[0])
        # print("ang vel w :", self._robot.data.root_ang_vel_w[0])
        # print("delta w   :", delta_ang_vel_w[0])
        # print("ang yaw al:", ang_vel_yaw_aligned[0])
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        lin_vel_error_mapped_walking = torch.exp(-lin_vel_error / 0.25) * velocity_scaling_multiple
        # yaw rate tracking
        yaw_rate_error = torch.square(self.commands[:, 2] - self._robot.data.root_ang_vel_b[:, 2])
        yaw_rate_error_mapped_walking = torch.exp(-yaw_rate_error / 0.25)

        # linear velocity tracking
        lin_vel_error_standing = torch.sum(torch.square(torch.stack([self.commands[:, 0] - self._robot.data.root_lin_vel_b[:, 2], self.commands[:, 1] - self._robot.data.root_lin_vel_b[:, 1]], dim=1)), dim=1)
        vel_cmd_magnitude = torch.linalg.norm(self.commands[:, :2], dim=1)
        velocity_scaling_multiple = torch.clamp(1.0 + 0.5 * (vel_cmd_magnitude - 1.0), min=1.0)
        ...
        lin_vel_error_mapped = torch.where(self.is_standing.bool(), lin_vel_error_mapped_standing, lin_vel_error_mapped_walking)
        yaw_rate_error_mapped = torch.where(self.is_standing.bool(), yaw_rate_error_mapped_standing, yaw_rate_error_mapped_walking)
        ...
        target_base_height = torch.where(self.is_standing.bool(), self.cfg.target_base_height, self.cfg.target_base_height-0.35)
        ...
        stand_orientation = 1-torch.exp(-stand_orientation_error / 0.25)

        quad_orientation_error = (-1 - (self._robot.data.projected_gravity_b[:, 2])) ** 2
        quad_orientation = 1-torch.exp(-quad_orientation_error / 0.25)
        ...
        joint_penalty = self.joint_limit_log_barrier(self._robot.data.joint_pos, self._robot.data.joint_pos_limits[:, :, 0], self._robot.data.joint_pos_limits[:, :, 1], margin_ratio=margin_ratio)
        joint_limit_penalty = torch.sum(joint_penalty, dim=1)   # (num_envs,)
        joint_torques_log_barrier = self.joint_limit_log_barrier(self._robot.data.applied_torque, -self._robot.data.joint_effort_limits, self._robot.data.joint_effort_limits, margin_ratio=margin_ratio)
        ...
        rewards = {
            # "stability_critic": stability_rewards,
            "locomotion_critic" : stand_rewards,
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        lin_vel_condition = (
            ...
            self.cfg.speed_curriclum_reward_threshold
            # * velocity_scaling
    ...
    def sample_commands(self, env_ids):
        ...
        mask = torch.rand(len(env_ids), device=self.device) < 1.1  # True/False 50:50
