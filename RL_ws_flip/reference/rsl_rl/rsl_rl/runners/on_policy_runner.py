...
from rsl_rl.algorithms import PPO, PPOCustom, PPOMultiCritic
...
from rsl_rl.storage import RolloutStorage, RolloutStorageCustom, RolloutStorageMultiCritic
from rsl_rl.utils import resolve_obs_groups
from rsl_rl.utils.logger import Logger

from isaaclab_rl.rsl_rl.exporter import export_custom_policy_as_onnx, export_policy_as_onnx
...
class OnPolicyRunner:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        for it in range(start_it, total_it):
            ...
            if it % self.cfg["save_interval"] == 0:
                ...
                # export_custom_policy_as_onnx(self.alg.policy, normalizer=self.alg.policy.actor_obs_normalizer, memory_normalizer=self.alg.policy.memory_obs_normalizer, path=os.path.join(self.logger.log_dir))
                export_policy_as_onnx(self.alg.policy, normalizer=self.alg.policy.actor_obs_normalizer, path=os.path.join(self.logger.log_dir))
    ...
    def _construct_algorithm(self, obs: TensorDict) -> PPO | PPOMultiCritic | PPOCustom:
        ...
        actor_critic: ActorCritic | ActorCriticRecurrent | ActorCriticCNN | ActorCriticCustom = actor_critic_class(
            obs, self.cfg["obs_groups"], self.env.num_actions, **self.policy_cfg
        ).to(self.device)
        print(self.env.num_actions)
        # Initialize the storage
        storage_class = eval(self.policy_cfg.pop("rollout_class_name"))
