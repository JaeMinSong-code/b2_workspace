...
class RolloutStorageMultiCritic:
    ...
    class Transition:
        ...
        def __init__(self) -> None:
            ...
            self.rewards: TensorDict | None = None
            self.dones: torch.Tensor | None = None
            self.values: TensorDict | None = None
            ...
            self.recoverability: torch.Tensor | None = None
            self.quad_weight: torch.Tensor | None = None
            self.biped_weight: torch.Tensor | None = None
    ...
    def __init__(
        ...
        self.recoverability = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.quad_weight = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.biped_weight = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
    ...
    def init_rewards_transition_buffer(self, reward_set_names: list[str]) -> None:
        self.critic_names = reward_set_names

        self.rewards = TensorDict(
            {name: torch.zeros(self.num_transitions_per_env, self.num_envs, device=self.device) for name in self.critic_names},
            batch_size=[self.num_transitions_per_env, self.num_envs],
            device=self.device,
        )

        self.values = TensorDict(
            {name: torch.zeros(self.num_transitions_per_env, self.num_envs, device=self.device) for name in self.critic_names},
            batch_size=[self.num_transitions_per_env, self.num_envs],
            device=self.device,
        )

        self.returns = TensorDict(
            {name: torch.zeros(self.num_transitions_per_env, self.num_envs, device=self.device) for name in self.critic_names},
            batch_size=[self.num_transitions_per_env, self.num_envs],
            device=self.device,
        )

        self.advantages = TensorDict(
            {name: torch.zeros(self.num_transitions_per_env, self.num_envs, device=self.device) for name in self.critic_names},
            batch_size=[self.num_transitions_per_env, self.num_envs],
            device=self.device,
        )
    ...
    def add_transition(self, transition: Transition) -> None:
        ...
        # print("self.values.batch_size:", self.values.batch_size)
        # print("self.values[self.step].batch_size:", self.values[self.step].batch_size)
        # print("transition.values.batch_size:", transition.values.batch_size)

        # for k in self.critic_names:
        #     print(f"{k} storage leaf:", self.values[self.step][k].shape)
        #     print(f"{k} transition leaf:", transition.values[k].shape)
        ...
        self.recoverability[self.step].copy_(transition.recoverability.view(-1, 1))
        self.quad_weight[self.step].copy_(transition.quad_weight.view(-1, 1))
        self.biped_weight[self.step].copy_(transition.biped_weight.view(-1, 1))
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        ...
        recoverabilities = self.recoverability.flatten(0, 1)
        quad_weights = self.quad_weight.flatten(0, 1)
        biped_weights = self.biped_weight.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                # advantages_batch = sum(advantages[name][batch_idx] for name in self.critic_names)
                recoverability_batch = recoverabilities[batch_idx]
                quad_weight_batch = quad_weights[batch_idx]
                biped_weight_batch = biped_weights[batch_idx]
