...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: B2LabFlatPPORunnerCfg):
    ...
    wall_start = time.time()   # 루프 시작 실제 시각 (현실 시간 기준점)
    compute_accum = 0.0        # sleep 제외 순수 연산 시간 누적 (raw RTF 계산용)
    try:
        while simulation_app.is_running():
            ...
            with torch.inference_mode():
                ...
                actor_obs = torch.cat([prop_obs, velocity_latent, priv_latent], dim=-1)
                actions = policy(actor_obs)
                _, _, _, _, obs_dict = env.step(actions)

            data_logger.log_step(step_count * dt, prop_obs=prop_obs, actions=actions)

            # 이번 스텝 순수 연산 시간(sleep 제외) 누적
            compute_accum += time.time() - start_time

            sim_time = step_count * dt                      # 시뮬레이션이 진행한 누적 시간(s)
            # 현실 시간에 맞춰 동기화: 시뮬이 현실보다 앞서면(=더 빠르면) 그만큼 delay
            ahead = (wall_start + sim_time) - time.time()   # >0 이면 시뮬이 현실보다 앞섬(빠름)
            if ahead > 0:
                time.sleep(ahead)

            # 약 1초마다 sim/현실 시간 차이 및 RTF 출력
            if step_count % max(1, round(1.0 / dt)) == 0:
                real_elapsed = time.time() - wall_start
                rtf_raw = sim_time / compute_accum if compute_accum > 0 else float("inf")
                tag = "sim이 빠름→realtime로 delay" if rtf_raw >= 1.0 else "sim이 느림(연산 병목)"
                print(f"[time] sim={sim_time:7.2f}s real={real_elapsed:7.2f}s "
                      f"ahead={ahead:+.3f}s  RTF(sim/compute)={rtf_raw:5.2f}x  {tag}")
    finally:
