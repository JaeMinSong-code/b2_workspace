...
class B2quadReward:
    ...
    def _scheduled_swing_mask(self):
        # desired_contact_states는 [0,1] 연속값(stance일 확률). 0.5 미만이면 swing 목표.
        # 정지(zero_hold) 시엔 desired_contact_states가 1.0으로 덮여 있어 전부 stance → swing mask 0.
        return self.env.desired_contact_states < 0.5   # [B,4] bool

    def _reward_penalty_swing_foot_xy_vel(self):
        dtype = self.env.foot_pos_b.dtype
        desired_swing = self._scheduled_swing_mask().to(dtype=dtype)
        foot_vel_xy_b = self.env.foot_rel_velocities_b[:, :, :2].to(dtype=dtype)
        foot_speed_xy = torch.linalg.vector_norm(foot_vel_xy_b, dim=-1)
        max_speed = 1.0
        excess_speed = torch.relu(foot_speed_xy - max_speed)
        swing_count = desired_swing.sum(dim=1).clamp_min(1.0)
        penalty = (torch.square(excess_speed) * desired_swing).sum(dim=1) / swing_count
        return penalty * (~self.env.reset_buf.bool()).to(dtype=dtype)